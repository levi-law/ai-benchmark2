...
export const benchmarkResults = mysqlTable("benchmark_results", {
  ...
  sampleLimit: int("sampleLimit").notNull(),
  status: mysqlEnum("status", ["pending", "running", "completed", "error"]).notNull().default("pending"),
  progress: int("progress").notNull().default(0),
  currentTask: text("currentTask"),
  arcEasy: float("arcEasy"),
  hellaswag: float("hellaswag"),
  truthfulqa: float("truthfulqa"),
